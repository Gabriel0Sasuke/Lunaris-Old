...
<head>
	...
	<title>Lunaris - Scans</title>
	<link rel="stylesheet" href="../css/cab.css">
	<link rel="stylesheet" href="../css/lista_scan.css">
</head>
<body>
	<main onclick="fecharpopup()">
		...
		<div id="principal">
			<h1>Lista de Scans</h1>
			<?php
			include "../php/connection.php";
			$dados = $sql->query("SELECT user_id, user_nome, user_usuario, user_email, user_senha, scan_descricao, scan_generos, scan_site, scan_responsavel, scan_discord FROM usuario WHERE user_cargo = 'scan'");
			...
			<table>
				...
				<tr>
					...
					<td><button id="editar" onclick="window.location.href='../php/editar_scan.php?id=<?php echo $row['user_id']; ?>'">Editar</button></td>
					<td><button id="excluir" onclick="window.location.href='../php/excluir_scan.php?id=<?php echo $row['user_id']; ?>'">Excluir</button></td>
				</tr>
				<?php } ?>
			</table>
			<?php
			} else {
				echo "<div id='aviso'>Nenhuma scan encontrada.</div>";
			} ?>
		</div>
